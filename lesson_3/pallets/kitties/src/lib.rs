...
pub mod pallet {
    ...
    */
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create(origin: OriginFor<T>) -> DispatchResult {                      // v0
        // pub fn create(origin: OriginFor<T>, name: [u8; 4]) -> DispatchResult {       // v1
        // pub fn create(origin: OriginFor<T>, name: [u8; 8]) -> DispatchResult {          // v2
            ...
            // 由于已经定义成了常量，可以根据 type 的 get 方法获得
            let price = T::KittyPrice::get();
            // T::Currency::reserve(&who, price)?;     // 当 token 数量不够时，可能会返回错误
            // 把 reserve 改成 transfer
            T::Currency::transfer(&who, &Self::get_account_id(), price, ExistenceRequirement::KeepAlive)?;     // 在转账时保证账户有最小的余额
        ...
        #[pallet::call_index(1)]
        #[pallet::weight(10_000)]
        pub fn breed(origin: OriginFor<T>, kitty_id_1: KittyId, kitty_id_2: KittyId) -> DispatchResult {                     // v0
        // pub fn breed(origin: OriginFor<T>, kitty_id_1: KittyId, kitty_id_2: KittyId, name: [u8; 4]) -> DispatchResult {      // v1
        // pub fn breed(origin: OriginFor<T>, kitty_id_1: KittyId, kitty_id_2: KittyId, name: [u8; 8]) -> DispatchResult {         // v2
            let who = ensure_signed(origin)?;
